...
Deno.test("selectStrategy: returns 'hyde' for short queries (≤5 words)", () => {
...
Deno.test("selectStrategy: returns 'multi_query' when history is deep (>2)", () => {
  // Even with a short query, deep history → multi_query
  const result = selectStrategy("¿Y luego?", null, 4);
  assertEquals(result, "multi_query");
});
