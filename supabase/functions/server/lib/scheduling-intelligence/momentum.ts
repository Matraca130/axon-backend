...
export async function computeStudyMomentum(
  ...
  // Defense-in-depth: filter reviews by user's own session IDs, not just RLS.
  const [sessionsLast7, sessionsLast14Res, streakData] =
    await Promise.all([
      db
        .from("study_sessions")
  ...
  const sessions14 = sessionsLast14Res.data ?? [];
  const sessionsLast14 = { count: sessions14.length };
  const sessionIds = sessions14.map((s: { id: string }) => s.id);

  // Fetch reviews only for this user's sessions (defense-in-depth alongside RLS)
  const reviewStats = sessionIds.length > 0
    ? await db
        .from("reviews")
        .select("grade")
