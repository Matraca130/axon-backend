...
export function validateCorrectAnswerInOptions(
  ...
  // Letter index match: "A" → 0, "B" → 1, etc.
  if (upper.length === 1 && upper >= "A" && upper <= "Z") {
    const idx = upper.charCodeAt(0) - 65;
    if (idx >= 0 && idx < options.length) return correctAnswer;
  }

  // Prefix match: an option starts with "A)" or "A."
  if (upper.length === 1) {
...
function validateTrueFalseAnswer(correctAnswer: string, options: string[]): string {
  ...
  if (TRUE_VALUES.has(lower)) return options[0];
  if (FALSE_VALUES.has(lower)) return options[1];
