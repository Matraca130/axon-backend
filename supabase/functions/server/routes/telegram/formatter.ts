...
export function formatFlashcardSummary(
  ...
  if (count > 15) {
    lines.push(`_(mostrando 15 de ${count})_`);
  }
...
export function formatProgressSummary(progress: ProgressData): string {
  ...
  if (progress.details?.length) {
    ...
    for (const d of progress.details.slice(0, 8)) {
      ...
      lines.push(`${bar} ${d.topic_name} _(${d.course_name})_`);
    }
  }
...
export function formatScheduleSummary(schedule: ScheduleData): string {
  ...
  for (const task of schedule.tasks.slice(0, 10)) {
    ...
    const strike = task.is_completed ? "~" : "";
    lines.push(`${icon} ${strike}${task.title}${strike} — ${date}`);
  }
...
export function formatKeywordDetail(keyword: KeywordData): string {
  ...
  if (keyword.connections?.length) {
    ...
    for (const conn of keyword.connections.slice(0, 8)) {
      const rel = conn.relationship ? ` _(${conn.relationship})_` : "";
...
export function formatSummaryPreview(
  ...
  if (wordCount) {
    lines.push(`_${wordCount} palabras_`);
    lines.push("");
  }
  ...
  if (content.length > 500) {
    lines.push("\n\u2026 _(resumen truncado, abrí la app para ver completo)_");
  }
...
export function truncateForTelegram(text: string): string {
  ...
  const truncated = text.slice(0, TG_MAX_CHARS - 10);
  const lastNewline = truncated.lastIndexOf("\n");

  if (lastNewline > TG_MAX_CHARS * 0.7) {
